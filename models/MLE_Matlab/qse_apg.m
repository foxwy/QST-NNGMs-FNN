...
%
%   2. Becker, S. R., Cand�s, E. J., & Grant, M. C. (2011). Templates for 
...
% common options
defaults.rho0 = 'white'; % initial density matrix
defaults.threshold_step = 0.001*sqrt(d)*d*eps; % stop when trace distance upper bound between current and previous iterate is less than this
defaults.threshold_fval = -0.001*f(f~=0)'*log(f(f~=0)); % stop when negative log likeilhood less than this
...
for i=1:opts.imax
    ...
    if ~isempty(opts.rho_star)
        ...
        stats.Fq(i) = real(trace(sqrtm(sqrtm(opts.rho_star)*rho_new*sqrtm(opts.rho_star)))^2);
        %stats.Fq(i) = real(opts.ket'*rho_new*opts.ket);
...
if exist('boot_stats','var')
    ...
    stats.Fq = [boot_stats.Fq;stats.Fq];
