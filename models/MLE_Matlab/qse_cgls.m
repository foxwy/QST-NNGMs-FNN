...
% process options
defaults = struct;
defaults.threshold_step = 0.001*sqrt(d)*d*eps; % stop when trace distance upper bound between current and previous iterate is less than this^M
defaults.threshold_fval = -0.001*f(f~=0)'*log(f(f~=0)); % stop when negative log likeilhood less than this^M
...
defaults.mincondchange = -Inf;  % >0 represents achieve
